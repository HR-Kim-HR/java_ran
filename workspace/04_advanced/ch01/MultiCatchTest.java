...
public class MultiCatchTest {
    ...
    void divide3(int num1, String[] arr) {
        try {
            int num2 = arr[0].trim().substring(0, 10).length(); // arrr[0]이 null일 경우 NullPointerException 발생, arr가 빈 배열일 경우 ArrayIndexOutOfBoundsException 발생
    ...
    void divide4(int num1, String[] arr) {
        if (arr != null & arr.length > 0 && arr[0] != null && arr[0].trim().length() > 0) {
            int num2 = arr[0].trim().length(); // arrr[0]이 null일 경우 NullPointerException 발생, arr가 빈 배열일 경우 ArrayIndexOutOfBoundsException 발생
            int result = num1 / num2; // arr[0]이 " "일 경우 ArithmeticException 발생
            System.out.println(num1 + " / " + num2 + " = " + result);
        }
    }
