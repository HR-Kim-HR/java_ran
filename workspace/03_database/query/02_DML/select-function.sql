...
-- 게시글 작성일을 년 월 일 요일 시 분 초 형식으로 조회        %y 는 두 자리 년도, %Y는 네 자리 년도
...
-- 최근 7일 이내에 가입한 회원 조회     now 는 시간단위 시분초 까지 나옴   curdate 는 날짜만 나옴
select id, name, created_at, curdate(), date_sub(curdate(), interval 7 day) as before_7days
from member
where created_at >= date_sub(curdate(), interval 7 day);

-- 가입한지 1개월이 지난 회원 조회
select *, date_sub(curdate(), interval 1 month) as before_1months
from member
where created_at < date_sub(curdate(), interval 1 month);

-- 가입한지 35일 12시간이 지난 회원 조회
select *, date_sub(curdate(), interval '34 12' DAY_HOUR) as before_1months
from member
...
-- 순서대로 체크해봐서 셋 중 null 이 아닌 값을 사용.        phone 이 null 이 아니면 phone 사용, phone 이 null 이고 email 이 null 아니면 email 사용
...
-- 전화번호 등록 여부에 따라 상태를 다르게 표시
SELECT name, IF(phone IS NULL, '연락처 없음', '연락처 있음') AS phone_status
FROM member;

-- 가입 연도가 1년이 넘었으면 '우수 회원', 한달이 넘었으면 '일반 회원', 그렇지 않으면 '신규 회원'을 출력
select
	name, 
	created_at,
	case
